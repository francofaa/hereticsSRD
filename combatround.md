...
Resolving a combat is done in **rounds**. Each round represents about 5-10 seconds.
...
2. Resolve [Actions](#actions). Movement is part of the action phase.
	- [Melee and Ranged Attacks](#melee-and-ranged-attacks)
	- [Disengaging](#disengaging)
	- [Spell casting](#spell-casting)
	- [Stunts](#stunts)
	- [Miscellaneous Actions](#miscellaneous-actions)
	- [Reactions](#reactions)
1. *If goons or enemies hit a breaking point:* [Morale checks](#morale-checks)
2. *If either side decides to retreat:* [Retreat](#retreat)
3. *If the retreating side is pursued:* [Pursuit](#pursuit)
...
> When monsters have a surprise round, rather than unleashing a barrage of attacks at the player characters, which can feel un-fun for the players, consider ways in which the monsters could gain the upper hand by either attaining an advantageous position (giving time for allies to appear behind the player characters, for example), depriving the player characters of a resource (by stunning, throwing a net, grappling, extinguishing arrows, etc), or affecting the terrain (splitting the party, cutting off exits, starting the timer on a deadly trap, casting an inconvenient but non-damaging spell, etc).
...
The winning side of initiative acts first, followed by the losing side. Movement is part of these actions and is limited to the maximum movement a creature can make. During a round, a character may make 1 action and 1 free action, if available. The defender may be able to make a reaction, if available.
...
When an attacker takes something out of deep inventory (slots 4 and above), they must roll +dex versus the DC equal to the item's position in their inventory. On a success, they do it as a free action and can take another action. On a failure, they retrieve the item anyway. 

When the attacker wants to switch items in their quick slots (1-3), this can be done as a free action. 
...
- force [morale checks](#morale-checks) 
